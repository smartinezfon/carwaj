...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { name, whatsapp_number, role, community_ids, new_password } = body as {
    name?: string;
  ...
  if (!target || target.company_id !== requester.company_id) {
  ...
  if (community_ids !== undefined) updates.community_ids = community_ids;
  ...
  if (new_password) {
    ...
    // Mark must_change_password so they're prompted to set their own on next login
